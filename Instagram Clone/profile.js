...
btnMessages.addEventListener('click', function() {
    console.log('shashsh'); // This line logs 'shashsh' to the console when btnMessages is clicked
    window.location.href = "messages.html"; // This line redirects to messages.html
});
let profileDpbtns = document.getElementById('profileDpbtns');
let editprofiteabsoclose = document.querySelector('.editprofiteabso');
profileDpbtns.addEventListener('click', function() {
    editprofiteabsoclose.style.display = 'none';
...
edit_profile.addEventListener('click',function(){
    editprofiteabsoclose.style.display = 'block';
})
let archiv_backbtnbar = document.getElementById('archiv_backbtnbar')
let archiv_aboso = document.querySelector('.archiv_aboso');
archiv_backbtnbar.addEventListener('click',function(){
   archiv_aboso.style.display = 'none';
})
let view_archive = document.getElementById('view_archive');
view_archive.addEventListener('click',function(){
    archiv_aboso.style.display = 'block';
    archiv_aboso.style.display = 'flex';
})
let addtools = document.querySelector('div.addtools');
let ad_tools = document.getElementById('ad_tools');
ad_tools.addEventListener('click', function(){
...
}
{
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const charCountSpan = document.getElementById('char-countforbio');
        const maxCharacters = 2000;
        function updateCharCount() {
            const text = textarea.value;
            const charCount = text.length;
...
if (closebtn_stori) {
    closebtn_stori.addEventListener('click', function() {
        let storyScreen = document.querySelector('.highlitescreen');
        if (storyScreen) {
...
{
    let higliteouter_box = document.getElementById('higliteouter_box');
if (higliteouter_box) {
    higliteouter_box.addEventListener('click', function() {
        console.log('Clicked higliteouter_box'); 
        let highlitescreen = document.querySelector('.highlitescreen');
        if (highlitescreen) {
            // Check the current display state and toggle it
            if (highlitescreen.style.display === 'none' || highlitescreen.style.display === '') {
                highlitescreen.style.display = 'block';
                highlitescreen.style.display = 'flex';  
            } else {
                highlitescreen.style.display = 'none';
            }
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Retrieve user profile data from localStorage or initialize it if not present
    ...
    // Function to update the bio text in userProfileData and print it
    const updateBio = () => {
        const bioTextarea = document.getElementById('biotextarea');
        const bioText = bioTextarea.value.trim(); // Get the trimmed bio text
        // Update bio field in userProfileData
        userProfileData[0].bio = bioText;
        // Save updated userProfileData to localStorage
        localStorage.setItem('userProfileData', JSON.stringify(userProfileData));
        // Print the updated bio text to the console
        console.log("Updated Bio:", userProfileData[0].bio);
    };
    // Call updateBio function initially to sync data
    updateBio();
    // Add input event listener to the bio textarea
    const bioTextarea = document.getElementById('biotextarea');
    bioTextarea.addEventListener('input', updateBio);
    // Add click event listener to the submit button
    const submitBtn = document.getElementById('editprofile-submit');
    submitBtn.addEventListener('click', function(event) {
        event.preventDefault(); // Prevent form submission if the button is inside a form
        // Call updateBio function to save the bio text
        updateBio();
        // Optionally, you can redirect the user or perform other actions after saving the bio text
        // window.location.href = 'redirect-url';
    });
    // Get the container for displaying the profile picture preview
    const profilePreviewBox = document.querySelector('#profile-dp-img');
    const profileBox = document.querySelector('.profile-box'); // corrected selector
    // Set default image URL and fallback image URL
    const defaultImageUrl = 'https://upload.wikimedia.org/wikipedia/commons/a/ac/Default_pfp.jpg?20200418092106';
    const fallbackImageUrl = '/Assets/Default_pfp.jpg'; // Replace with your actual fallback image URL
    // Retrieve the stored profile picture URL from local storage
    const storedImageUrl = localStorage.getItem('previewImageUrl');
    // Set the profile picture URL to the stored URL or the default URL if not set
    let profileImageUrl = storedImageUrl || defaultImageUrl;
    // Display the profile picture
    profilePreviewBox.innerHTML = `<img id="profileImgElement" src="${profileImageUrl}" style="display:block;">`;
    // Update the profile picture in the profile_box
    profileBox.innerHTML = `<img src="${profileImageUrl}" alt="">`;
    // If no stored profile picture URL, set the default image
    if (!storedImageUrl) {
        document.getElementById('profileImgElement').src = defaultImageUrl;
    }
    // Add error event listener to handle image loading errors
    const profileImgElement = document.getElementById('profileImgElement');
    profileImgElement.addEventListener('error', function() {
        // Display the fallback image if the main image fails to load
        profileImgElement.src = fallbackImageUrl;
    });
    ...
    dpInput.addEventListener('change', function() {
        ...
        if (selectedFile) {
            ...
            if (profileImageUrl !== defaultImageUrl) {
                URL.revokeObjectURL(profileImageUrl);
            }
            // Save the new image URL as the profile picture
            localStorage.setItem('previewImageUrl', fileUrl);
            profileImageUrl = fileUrl;
            // Update the profile picture preview with the selected file URL
            document.getElementById('profileImgElement').src = fileUrl;
            profileBox.innerHTML = `<img src="${fileUrl}" alt="">`; // Corrected to display selected profile picture
            // Update the dp field in userProfileData with the new image URL
            userProfileData[0].dp = fileUrl;
            // Print the updated URL of the profile picture (dp) to the console
            console.log("Updated Profile Picture URL (dp):", userProfileData[0].dp);
        } else {
